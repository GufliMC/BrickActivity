...
import com.guflimc.brick.arena.spigot.events.ActivityEvent;
...
class BaseEventSubscription<T extends Event> extends EventSubscription implements Listener, EventExecutor {
    ...
    private BaseEventSubscription(@NotNull SpigotActivity activity,
    ...
    public void execute(@NotNull Listener listener, @NotNull Event event) {
        ...
        if (event instanceof EntityEvent ee && activity.arenas().stream().noneMatch((a) -> a.contains(SpigotMath.toBrickLocation(ee.getEntity().getLocation())))) {
            return;
        }
        if (event instanceof BlockEvent be && activity.arenas().stream().noneMatch((a) -> a.contains(SpigotMath.toBrickLocation(be.getBlock().getLocation())))) {
            return;
        }
    ...
    static class Builder<T extends Event> extends EventSubscription.Builder<T> {
        ...
        Builder(@NotNull SpigotActivity activity, @NotNull Class<T> eventClass) {
            this.activity = activity;
            this.eventClass = eventClass;
        }
        ...
        public EventSubscription subscribe() {
            ...
            BaseEventSubscription<T> sub = new BaseEventSubscription<>(activity, eventClass, handler, until, conditions);
